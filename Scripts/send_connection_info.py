...
def send_qp_info(client_ip_addr="22.1.212.11", fpga_ip_addr="22.1.212.10", fpga_qpn=0x11, client_qpn=0x12,
                 close_qp=False, psn=0x0, r_key=0x0, rem_base_addr=0x0):
    ...
    qpinfo_flags = 0x1 | ((not close_qp) << 1)

    MESSAGE = b''

    MESSAGE += struct.pack("<B", qpinfo_flags)  # QP_info_valid and open qp
...
def send_txmeta(client_ip_addr="22.1.212.11", fpga_ip_addr="22.1.212.10", fpga_qpn=0x11, client_qpn=0x12,
                rem_addr_offset=0x0, rdma_length=0x0, start_flag=0, immd_flag=0, txtype_flag=1):
    REM_IP_ADDRESS = client_ip_addr
    REM_IP_ADDRESS_INT = int(ip_address(client_ip_addr))
    FPGA_QPN = fpga_qpn
    CLIENT_QPN = client_qpn
    ...
    MESSAGE += struct.pack('<L', 0x0)[:3]  # client_qpn
    MESSAGE += struct.pack('<L', FPGA_QPN)[:3]  # fpga_qpn
...
if __name__ == "__main__":
    ...
    if args.start:
        send_txmeta(client_ip_addr=args.client_ip_addr, fpga_ip_addr=args.fpga_ip_addr, fpga_qpn=args.qpnf,
                    rem_addr_offset=args.addrOffset, rdma_length=args.length, start_flag=args.start,
                    immd_flag=args.immediate, txtype_flag=args.txtype)
    else:
        send_qp_info(client_ip_addr=args.client_ip_addr, fpga_ip_addr=args.fpga_ip_addr, fpga_qpn=args.qpnf,
                     client_qpn=args.qpnc, close_qp=args.close, psn=args.psn, r_key=args.rkey,
                     rem_base_addr=args.BaseAddr)
