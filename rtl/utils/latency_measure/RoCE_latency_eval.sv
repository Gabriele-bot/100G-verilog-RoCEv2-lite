...
module RoCE_latency_eval (
  ...
  output wire        transfer_time_inst_valid,
  ...
  assign latency_inst_valid = latency_out_inst_valid;
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      if (last_frame && s_axis_tx_payload_valid && s_axis_tx_payload_last) begin
        ...
        throughput_out_inst_valid <= 1'b1;
      end
    end
  end
  ...
  assign transfer_time_inst_valid = throughput_out_inst_valid;
